...
%               (Vr) is used
% * bmin:       lower bound for the model parameter b; if bmin is empty, 
%               zero is used 
...
% * fr:         firing rate in Hz
    ...
    warning off;
    [Cm,gL,EL,sf,Vup,tcw,a,b,Vr,Vth]=names(values);
    ...
    if (X_Vth<=0 || f>=1 || b<bmin || Cm<=0 || gL<=0 || tcw<=0 || sf<=0 )
        ...
        if V_r >= Vth
            if w_r>=wV_Vr
                ...
                if (isempty(Vfix) || length(Vfix)==1)
                    t1=quadl(@ISI_int_piecewise,V_r,Vth,[],[],I,values,i,j,k);
                    t2=0;
                else
                    Vlb=min(Vfix);
                    t1=quadl(@ISI_int_piecewise,V_r,Vlb,[],[],I,values,i,j,k);
                    m=f*gL-gL;
                    i=m; j=(1-f)*I-m*EL; k=(1-f)*gL*sf;
                    t2=quadl(@ISI_int_piecewise,Vlb,Vth,[],[],I,values,i,j,k);
                end
                w_stop=w_end;
                Vlb=Vth;
            else
            ...
            if(w_r < w2 && w_r > w1)
                m=f*gL-gL;
                i=m; j=(1-f)*I-m*EL; k=(1-f)*gL*sf;
                t2=quadl(@ISI_int_piecewise,V_r,Vth,[],[],I,values,i,j,k);
                w_stop=w_end;
            else
                i=0; j=w_r; k=0;
                if (w_r <= w1)
        ...
        if Vlb>=Vup
            ...
            i=0; j=w_stop; k=0;
            t3=quadl(@ISI_int_piecewise,Vlb,Vup,[],[],I,values,i,j,k);
        end
